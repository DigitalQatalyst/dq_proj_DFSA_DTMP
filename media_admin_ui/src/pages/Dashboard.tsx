...
import { PlusIcon, EditIcon, ClockIcon, CheckCircleIcon, ArchiveIcon, XIcon } from 'lucide-react';
...
const StatusCard: React.FC<StatusCardProps> = ({
  ...
  onClick,
  isActive = false
}) => <div className={`bg-white rounded-lg shadow p-6 border-t-4 ${color} ${onClick ? 'cursor-pointer hover:shadow-md transition-shadow' : ''} ${isActive ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`} onClick={onClick} tabIndex={onClick ? 0 : undefined} role={onClick ? 'button' : undefined} aria-pressed={isActive ? 'true' : undefined} onKeyDown={e => {
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const {
          data
        } = await mediaService.getMediaItems({}, 1, 5, {
          field: 'updatedAt',
          direction: 'desc'
        });
        setRecentItems(data);
  ...
  const handleStatusCardClick = (status: string) => {
    // Navigate to media list with filter (admin-ui base)
  ...
  return <AppLayout title="Dashboard">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">
          Welcome to the KF eJP Library Media Hub
        </p>
      </div>
      {loading ? <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
        </div> : <>
          {activeFilter && <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mb-4 flex justify-between items-center">
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div className="divide-y divide-gray-200">
              {recentItems.length === 0 ? <div className="px-6 py-4 text-center text-gray-500">
                  No recent activity found.
                </div> : recentItems.map(item => <div key={item.id} className="px-6 py-4 hover:bg-gray-50">
