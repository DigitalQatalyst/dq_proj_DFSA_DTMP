...
const MediaCreate: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Submitting media item form data:', formData);
      const newMediaItem = await mediaService.createMediaItem({
        ...formData
      });
      // Show success toast
      setToast({
      ...
      // Navigate to the new media item page after a short delay
      setTimeout(() => {
        navigate(`/admin-ui/media/${newMediaItem.id}`);
      }, 1500);
    } catch (error) {
      // Log the detailed error
      console.error('Error creating media item:', {
        error,
        formData
      });
      // Show error toast with the actual error message
      setToast({
        message: `Failed to create media item: ${error instanceof Error ? error.message : 'Unknown error'}`,
        type: 'error'
      });
      setSubmitting(false);
    }
  ...
  const validateStep1 = () => {
    return formData.title && formData.slug && formData.type && formData.summary;
  };
  return <AppLayout title="Create New Media Item">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          
          <p className="text-gray-500 mt-1">
            Add a new piece of content to your knowledge library
          </p>
        </div>
        {/* Toast notification */}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center">
            <div className="flex items-center">
              <div className={`flex items-center justify-center h-10 w-10 rounded-full ${step >= 1 ? 'bg-blue-600' : 'bg-gray-300'} text-white`}>
            ...
            <div className="flex items-center">
              <div className={`flex items-center justify-center h-10 w-10 rounded-full ${step >= 2 ? 'bg-blue-600' : 'bg-gray-300'} text-white`}>
            ...
            {step === 1 && <div className="p-6">
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      A brief summary of the content. Will be used in listings
                      and previews.
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      The main content of your media item. Supports basic HTML
                      formatting.
                    </p>
                  </div>
                </div>
                <div className="mt-6 flex justify-end">
                  <button type="button" onClick={() => setStep(2)} disabled={!validateStep1()} className={`px-4 py-2 rounded-md text-white font-medium ${validateStep1() ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-300 cursor-not-allowed'}`}>
                    Next: SEO & Metadata
                  </button>
                </div>
              </div>}
            {step === 2 && <div className="p-6">
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Public items are visible to all users. Private items are
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Optional. If left empty, the regular title will be used.
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Optional. If left empty, the summary will be used.
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Optional. Use this if the content exists elsewhere and you
                      want to avoid duplicate content issues.
                    </p>
                  </div>
                </div>
                <div className="mt-6 flex justify-between">
                  <button type="button" onClick={() => setStep(1)} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 bg-white hover:bg-gray-50">
                    Back: Basic Information
                  </button>
                  <button type="submit" disabled={submitting} className={`px-4 py-2 rounded-md text-white font-medium ${submitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                    {submitting ? 'Creating...' : 'Create Media Item'}
                  </button>
                </div>
              </div>}
          </form>
