/**
 * Integrations & Billing Tab Component
 *
 * Manages external integrations, API tokens, and billing settings.
 *
 * @component
 * @example
 * ```tsx
...
import { Integration, ApiToken } from '../types/integrations';
...
export default function IntegrationsBillingTab() {
  ...
  const getIconComponent = (iconName: string) => {
    switch (iconName) {
      ...
      case 'bar-chart':
        return <BarChartIcon className="h-6 w-6 text-blue-600" />;
      case 'shopping-cart':
  ...
  return <div className="space-y-6">
      ...
      <section className="opacity-60">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="md:hidden" onClick={() => toggleSection('integrations')} aria-expanded={expandedSection === 'integrations'} aria-controls="integrations-section">
            {expandedSection === 'integrations' ? <ChevronUpIcon className="h-5 w-5 text-gray-500" aria-hidden="true" /> : <ChevronDownIcon className="h-5 w-5 text-gray-500" aria-hidden="true" />}
          </button>
        </div>
        {/* Desktop view: always visible */}
        <div id="integrations-section" className={`space-y-4 ${expandedSection === 'integrations' || !isMobile() ? 'block' : 'hidden md:block'}`}>
      ...
      <section className="opacity-60">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center">
            ...
            <button className="md:hidden ml-2" onClick={() => toggleSection('api-tokens')} aria-expanded={expandedSection === 'api-tokens'} aria-controls="api-tokens-section">
              {expandedSection === 'api-tokens' ? <ChevronUpIcon className="h-5 w-5 text-gray-500" aria-hidden="true" /> : <ChevronDownIcon className="h-5 w-5 text-gray-500" aria-hidden="true" />}
            </button>
          </div>
        </div>
        {/* Desktop Table View */}
        <div id="api-tokens-section" className={`hidden md:block bg-white overflow-hidden border border-gray-200 rounded-md`}>
              ...
              <div className="px-4 py-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="relative">
                    <button onClick={e => toggleActionMenu(token.id, e)} className="p-1 text-gray-500 hover:text-gray-700 focus:outline-none" aria-label="Token actions" aria-expanded={openActionMenuId === token.id} aria-haspopup="true">
                      <MoreVerticalIcon className="h-5 w-5" />
                    </button>
                    {/* Action Menu */}
                    {openActionMenuId === token.id && <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200" role="menu" aria-orientation="vertical">
                        <div className="py-1">
                          <button disabled className="flex items-center px-4 py-2 text-sm text-gray-400 w-full text-left cursor-not-allowed" role="menuitem">
      ...
      <section className="opacity-60 cursor-not-allowed">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="md:hidden" onClick={() => toggleSection('billing')} aria-expanded={expandedSection === 'billing'} aria-controls="billing-section">
            {expandedSection === 'billing' ? <ChevronUpIcon className="h-5 w-5 text-gray-500" aria-hidden="true" /> : <ChevronDownIcon className="h-5 w-5 text-gray-500" aria-hidden="true" />}
          </button>
        </div>
        <div id="billing-section" className={`${expandedSection === 'billing' || !isMobile() ? 'block' : 'hidden md:block'}`}>
