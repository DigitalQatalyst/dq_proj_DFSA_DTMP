  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        bodyHtml: DOMPurify.sanitize((formData as any).bodyHtml || formData.body || ''),
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-6 space-y-6">
              ...
              <div className="mt-1">
                <Suspense fallback={<div className="border rounded-md p-3 text-sm text-gray-500">Loading editorâ€¦</div>}>
                  <RichTextEditor
                    ...
                    onChange={(json, html) => setFormData((prev) => ({ ...(prev as any), bodyJson: json, bodyHtml: html }))}
                  />
                </Suspense>
              </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-2">Paste a URL or upload a new image. Optional.</p>
              <div className="flex items-center gap-2 mb-3">
                <input type="url" value={thumbnailUrl} onChange={(e) => { setThumbnailUrl(e.target.value); setPendingThumbFile(null); setThumbnailError(null) }} placeholder="https://example.com/image.jpg" className="flex-1 border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                <input type="file" accept="image/*" className="text-sm" onChange={(e) => { const f = e.target.files?.[0] || null; if (f) { setPendingThumbFile(f); setThumbnailError(null) } }} />
              </div>
              {(thumbnailUrl || pendingThumbFile) && (
                <div className="border rounded-md p-2 bg-gray-50">
                  <div className="h-40 w-full overflow-hidden rounded-md bg-white">
                    {pendingThumbFile ? (
                      <img src={URL.createObjectURL(pendingThumbFile)} alt={(formData.title || 'Thumbnail preview')} className="h-40 w-full object-cover" />
                    ) : (
                      <img src={thumbnailUrl} alt={(formData.title || 'Thumbnail preview')} className="h-40 w-full object-cover" />
                    )}
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <button type="button" className="px-2 py-1 border rounded-md" onClick={() => { setThumbnailUrl(''); setPendingThumbFile(null) }}>Remove</button>
                  </div>
                  {thumbnailError && <div className="mt-2 text-xs text-red-600">{thumbnailError}</div>}
                </div>
              )}
            </div>
